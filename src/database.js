...
async function addSampleTool() {
  ...
  // Create a sample tokens/words counter tool
  const sampleTool = {
    name: 'tokens_words_counter.py',
    title: 'Tokens & Words Counter',
    description: 'Analyzes a text file to count tokens, words, and estimate reading time.',
    options: [
      {
        name: 'input_file',
        label: 'Input File',
      ...
      },
      {
        ...
        type: 'boolean',
        description: 'Include additional details in the analysis',
        default: false
      },
      {
        name: 'reading_speed',
        ...
        type: 'number',
        description: 'Reading speed in words per minute for time estimation',
        default: 250,
        min: 100,
        max: 800,
        step: 10
  ...
  await database.addOrUpdateTool(sampleTool);
  console.log('Sample tool added');
...
// Add the sample tool on initialization
addSampleTool();
